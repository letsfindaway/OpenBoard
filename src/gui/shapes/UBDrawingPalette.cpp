    ...
    : UBActionPalette(Qt::TopLeftCorner, parent, orient)
{
    ...
    addButtonSubPalette(new UBDrawingStrokePropertiesPalette(Qt::Horizontal, parentWidget()), shapeActions->actionStrokeProperties);
    addButtonSubPalette(new UBDrawingFillPropertiesPalette(Qt::Horizontal, parentWidget()), shapeActions->actionFillProperties);
    addButtonSubPalette(new UBDrawingArrowsPropertiesPalette(Qt::Horizontal, parentWidget()), shapeActions->actionArrowProperties);
    UBActionPaletteButton * btnPaintBucket = addActionButton(shapeActions->actionChangeFillingColor);

    //Sankore-1701 - NNE - 20140324
    addButtonSubPalette(new UBAlignObjectPalette(Qt::Horizontal, parentWidget()), shapeActions->actionAlignObject);
...
void UBDrawingPalette::initPosition()
{
    ...
    int indexDrawingButton = stylusPalette->actions().indexOf(UBApplication::boardController->shapeFactory().shapeActions()->actionDrawing);
    ...
    if (buttonsDrawing.size()>0)
    {
        ...
        if (buttonDrawing)
        {
            // orientation() may be reimplemented in UBActionPalette
